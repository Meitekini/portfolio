...
import {
  ...
  const NavBar = () => {
    ...
    return (
      ...
      // gap-y-4 fixed h-max bottom-0 mt-auto xl:right-[2%] z-50 top-0 w-full
      // xl:w-16 xl:max-w-md xl:h-screen bg-red-500/30
      // "
    ...
    '>
        ...
        ">
        {navData.map((item)=>(
          <Link key={item.name} href={item.path} className={`${pathname === item.path && 'text-accent'} relative flex items-center group hover:text-accent transition-all 
           duration-300`}>
             {/*tooltip  */}
